...
- 另一個建置環境的做法：`npx create-react-app dir_name`，會自動幫你蓋環境，然後下 `npm run start`，server 就會自己跑起來
...
class receiver extends Component {
  render(){
    return (
      <h1>{this.props.number}</h1> // props：別人傳過來的；state：自己的
    )
  } 
}
